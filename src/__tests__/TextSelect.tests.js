...
test("TextSelect renders an article without initial tags", () => {
  const { getByTestId } = render(<TextSelect article={article} />);
  expect(getByTestId("notags").textContent).toEqual(
